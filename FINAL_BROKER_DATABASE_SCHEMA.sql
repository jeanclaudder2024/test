-- =================================================================
-- FINAL BROKER DATABASE SCHEMA - PRODUCTION READY
...
-- 2. CREATE BROKER DEALS TABLE (MAIN TABLE)
...
-- 3. CREATE DEAL MESSAGES TABLE (WITH RECEIVER_ID)
...
-- 4. CREATE TRANSACTION STEPS TABLE
...
-- 5. CREATE TRANSACTION DOCUMENTS TABLE
...
-- 6. CREATE BROKER CARD APPLICATIONS TABLE
...
-- 7. CREATE BROKER DOCUMENTS TABLE
...
-- 8. CREATE BROKER ADMIN FILES TABLE
...
-- 10. CREATE BROKER PROFILES TABLE
...
-- Ensure deal_status column exists in broker_deals
ALTER TABLE broker_deals 
ADD COLUMN IF NOT EXISTS deal_status VARCHAR(50) DEFAULT 'draft';

-- Ensure receiver_id column exists in deal_messages  
...
-- =================================================================
-- VERIFICATION - RUN THIS TO CHECK SUCCESS
-- =================================================================

SELECT 
  'SUCCESS: All broker tables created!' as result,
...
SELECT 
  CASE 
    WHEN COUNT(*) = 4 THEN '✅ All critical columns exist'
    ELSE '❌ Missing columns - check errors above'
  END as column_status
FROM information_schema.columns
WHERE table_name IN ('broker_deals', 'deal_messages')
AND column_name IN ('status', 'deal_status', 'receiver_id', 'transaction_type');