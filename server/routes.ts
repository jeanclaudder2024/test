...
Note: This document contains real vessel operational data and should be treated as confidential maritime information.`;
  ...
  // ==========================================
  // VESSEL TYPES MANAGEMENT API ROUTES
  // ==========================================

  // Get all vessel types
  apiRouter.get("/admin/vessel-types", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
    try {
      const vesselTypes = await storage.getVesselTypes();
  ...
  apiRouter.get("/admin/vessel-types/:id", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
    try {
      const vesselTypeId = parseInt(req.params.id);
      
      if (isNaN(vesselTypeId)) {
        return res.status(400).json({ message: "Invalid vessel type ID" });
      }

      const vesselType = await storage.getVesselTypeById(vesselTypeId);
      
      if (!vesselType) {
        return res.status(404).json({ message: "Vessel type not found" });
      }
      ...
      res.status(500).json({ 
        message: "Failed to fetch vessel type",
        error: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });
  ...
  apiRouter.post("/admin/vessel-types", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const newVesselType = await storage.createVesselType({
        name,
        description,
        category: category || 'General'
      });

      res.status(201).json(newVesselType);
    } catch (error) {
      console.error("Error creating vessel type:", error);
      res.status(500).json({ 
        message: "Failed to create vessel type",
        error: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });
  ...
  // Update vessel type
  apiRouter.put("/admin/vessel-types/:id", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
    try {
      const vesselTypeId = parseInt(req.params.id);
      ...
      const updatedVesselType = await storage.updateVesselType(vesselTypeId, {
        name,
        description,
        category: category || 'General'
      });
      
      if (!updatedVesselType) {
        return res.status(404).json({ message: "Vessel type not found" });
      }

      res.json(updatedVesselType);
    } catch (error) {
      console.error("Error updating vessel type:", error);
      res.status(500).json({ 
        message: "Failed to update vessel type",
        error: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });
  ...
  // Delete vessel type
  apiRouter.delete("/admin/vessel-types/:id", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res) => {
    try {
      const vesselTypeId = parseInt(req.params.id);
      ...
      const deleted = await storage.deleteVesselType(vesselTypeId);
      
      if (!deleted) {
        return res.status(404).json({ message: "Vessel type not found" });
      }

      res.json({ message: "Vessel type deleted successfully" });
    } catch (error) {
      console.error("Error deleting vessel type:", error);
      res.status(500).json({ 
        message: "Failed to delete vessel type",
        error: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });
