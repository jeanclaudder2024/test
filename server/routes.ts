...
Only use authentic, real-world data for existing refineries.`;
  ...
  app.post("/api/confirm-broker-membership", authenticateToken, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      // Accept both 'succeeded' and 'requires_payment_method' for test mode
      const validStatuses = ['succeeded', 'requires_payment_method', 'processing'];
      ...
      if (validStatuses.includes(paymentIntent.status)) {
        ...
        const autoMembershipCard = {
          userId: req.user.id,
          membershipId,
          cardNumber,
          firstName: req.user.firstName || 'Professional',
          lastName: req.user.lastName || 'Broker',
          status: 'approved',
