...
Keep the description professional, informative, and around 150-200 words. Focus on factual maritime industry details.`;
  ...
  // Get vessels connected to a specific port
  apiRouter.get("/ports/:portId/vessels", async (req, res) => {
    try {
      const portId = parseInt(req.params.portId);
      ...
      // Get vessels near this port (within 0.5 degrees)
      const vessels = await storage.getVessels();
      const nearbyVessels = vessels.filter(vessel => {
        if (!vessel.currentLat || !vessel.currentLng) return false;
      ...
      console.error("Error getting port vessels:", error);
      res.status(500).json({ 
        message: "Failed to get port vessels",
        error: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });

  // Get nearby refineries for a port
  apiRouter.get("/ports/:portId/nearby-refineries", async (req, res) => {
    try {
      const portId = parseInt(req.params.portId);
      ...
      // Get refineries in the same region or nearby
      const refineries = await storage.getRefineries();
      const nearbyRefineries = refineries.filter(refinery => {
        // Same region or calculate distance
        if (refinery.region === port.region) return true;
        ...
        try {
          ...
          return distance <= 2.0; // Within 2 degrees
        } catch (error) {
          return false;
        }
      });
      ...
      console.error("Error getting nearby refineries:", error);
      res.status(500).json({ 
        message: "Failed to get nearby refineries",
        error: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });
  ...
  apiRouter.post("/ports/:portId/connect-vessel", async (req, res) => {
    try {
      const portId = parseInt(req.params.portId);
      const { vesselId, connectionType } = req.body;
      
      if (isNaN(portId) || !vesselId) {
        return res.status(400).json({ message: "Invalid port ID or vessel ID" });
      }
      ...
      // Update vessel location to be near the port
      await storage.updateVessel(vesselId, {
        currentLat: port.lat,
        currentLng: port.lng,
        status: connectionType || 'Docked'
      });

      res.json({ 
        success: true,
        message: "Vessel connected to port successfully"
      });
