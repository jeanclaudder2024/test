...
export function createVesselRefineryRouter(storage: IStorage) {
  const vesselRefineryRouter = Router();

  // Get all vessel-refinery connections
  vesselRefineryRouter.get("/", async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: "Failed to fetch vessel-refinery connections" });
    }
  });

  // Get a specific vessel-refinery connection by ID
  vesselRefineryRouter.get("/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.error("Error fetching vessel-refinery connection:", error);
      res.status(500).json({ error: "Failed to fetch vessel-refinery connection" });
    }
  });

  // Get connections for a specific vessel
  vesselRefineryRouter.get("/vessel/:vesselId", async (req: Request, res: Response) => {
    try {
      const vesselId = parseInt(req.params.vesselId);
      if (isNaN(vesselId)) {
        return res.status(400).json({ error: "Invalid vessel ID format" });
      }
      ...
      res.status(500).json({ error: "Failed to fetch vessel connections" });
    }
  });

  // Get connections for a specific refinery
  vesselRefineryRouter.get("/refinery/:refineryId", async (req: Request, res: Response) => {
    try {
  ...
  vesselRefineryRouter.post("/", async (req: Request, res: Response) => {
    try {
      const { vesselId, refineryId, connectionType, status, cargoVolume, startDate, endDate } = req.body;

      if (!vesselId || !refineryId) {
        return res.status(400).json({ error: "Vessel ID and Refinery ID are required" });
      }
      ...
      res.status(500).json({ error: "Failed to create vessel-refinery connection" });
    }
  });

  // Update a vessel-refinery connection
  vesselRefineryRouter.put("/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ error: "Invalid connection ID format" });
      ...
      res.status(500).json({ error: "Failed to update vessel-refinery connection" });
    }
  });

  // Delete a vessel-refinery connection
  vesselRefineryRouter.delete("/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ error: "Invalid connection ID format" });
      ...
      res.status(500).json({ error: "Failed to delete vessel-refinery connection" });
    }
  });

  // Connect vessel to nearby refineries automatically
  vesselRefineryRouter.post("/connect-vessel/:vesselId", async (req: Request, res: Response) => {
    try {
      const vesselId = parseInt(req.params.vesselId);
      if (isNaN(vesselId)) {
        return res.status(400).json({ error: "Invalid vessel ID format" });
      ...
      if (!vessel.currentLat || !vessel.currentLng) {
        return res.status(400).json({ error: "Vessel does not have current coordinates" });
      }
      ...
      res.status(500).json({ error: "Failed to connect vessel to refineries" });
    }
  });

  return vesselRefineryRouter;
}