-- Fix landing page content schema to match the code expectations
-- Drop the old table and recreate with the correct schema

BEGIN;

-- Drop the existing table that has wrong schema
DROP TABLE IF EXISTS landing_page_content CASCADE;

-- Create the correct landing_page_content table matching the schema.ts definition
...
-- Insert sample landing page content for management
