-- Final Company Management System for Supabase
...
-- Insert sample real companies (using existing columns only)
INSERT INTO companies (name, country, region, company_type, specialization, fleet_size, employees, publicly_traded) VALUES
('ExxonMobil Corporation', 'United States', 'North America', 'real', 'Crude Oil', 45, 72000, true),
('Shell plc', 'Netherlands', 'Europe', 'real', 'Mixed Fleet', 38, 82000, true),
('BP plc', 'United Kingdom', 'Europe', 'real', 'Mixed Fleet', 42, 70100, true),
('TotalEnergies SE', 'France', 'Europe', 'real', 'Mixed Fleet', 35, 105000, true),
('Chevron Corporation', 'United States', 'North America', 'real', 'Crude Oil', 33, 45600, true)
...
SELECT 
    'Global Energy Solutions Ltd', 'Singapore', 'Asia-Pacific', 'fake', id, 'Crude Oil', 15, 2500, true
FROM companies WHERE name = 'ExxonMobil Corporation' AND company_type = 'real'
ON CONFLICT (name) DO NOTHING;

INSERT INTO companies (name, country, region, company_type, linked_company_id, specialization, fleet_size, employees, is_visible_to_brokers)
SELECT 
    'Atlantic Maritime Corp', 'Liberia', 'Africa', 'fake', id, 'Mixed Fleet', 12, 1800, true
FROM companies WHERE name = 'Shell plc' AND company_type = 'real'
ON CONFLICT (name) DO NOTHING;

INSERT INTO companies (name, country, region, company_type, linked_company_id, specialization, fleet_size, employees, is_visible_to_brokers)
SELECT 
    'Pacific Oil Transport', 'Panama', 'Latin America', 'fake', id, 'Refined Products', 8, 1200, true
