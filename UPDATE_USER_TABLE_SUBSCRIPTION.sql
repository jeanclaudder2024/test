-- Add subscription-related columns to users table for easier access
-- IMPORTANT: This preserves the admin role with full access

-- Add isSubscribed column to users table
ALTER TABLE users 
...
-- Add subscription_id to track active subscription
ALTER TABLE users
ADD COLUMN IF NOT EXISTS subscription_id INTEGER REFERENCES user_subscriptions(id);

-- Update existing admin users to have full access (no subscription required)
UPDATE users 
SET is_subscribed = true,
    subscription_tier = 'admin'
WHERE role = 'admin';

-- Create a function to check if user has active subscription or is admin
...
-- Create a trigger to update user subscription status when subscription changes
...
-- Ensure admin users always have the admin tier
UPDATE users
SET is_subscribed = true,
    subscription_tier = 'admin'
WHERE role = 'admin';