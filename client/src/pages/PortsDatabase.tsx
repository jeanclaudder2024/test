...
// Port types and other constants
const portTypes = [
  "Oil Terminal",
  "Container Port", 
  "Bulk Terminal",
...
const countries = [
  ...
  "Russia", "Iran", "Iraq", "Libya", "Algeria", "Venezuela"
];
...
const countryToFlagCode: { [key: string]: string } = {
  ...
  "Nigeria": "NG", "Egypt": "EG", "Russia": "RU", "Iran": "IR",
  "Iraq": "IQ", "Libya": "LY", "Algeria": "DZ", "Venezuela": "VE"
};
...
export default function PortsDatabase() {
  ...
  // Fetch ports from your database
  const { data: ports = [], isLoading, error, refetch } = useQuery<Port[]>({
    queryKey: ["/api/ports"],
    queryFn: async () => {
      const response = await fetch("/api/ports");
      if (!response.ok) throw new Error("Failed to fetch ports from database");
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center">
            <div>
              ...
              <p className="text-gray-600 mt-2 max-w-2xl text-sm">
                Professional port management system connected to your authentic database
              </p>
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>Authentic Ports Database ({filteredPorts.length})</span>
                          ...
                          {port.capacity ? (
                            <div className="text-sm">
                              <div className="font-medium">
                                {port.capacity.toLocaleString()}
                              </div>
                              <div className="text-gray-500">TEU/Barrels</div>
                            </div>
                          ) : (
                            <span className="text-gray-400">—</span>
                          )}
